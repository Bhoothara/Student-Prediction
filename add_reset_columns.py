...
if 'users' not in tables:
    print("WARNING: 'users' table not found. Please check table name and tell me.")
    conn.close()
    sys.exit(1)
...
if "reset_code_hash" not in cols:
    ...
    print("Added column: reset_code_hash")
else:
    print("Column already exists: reset_code_hash")

if "reset_code_expiry" not in cols:
    cur.execute("ALTER TABLE users ADD COLUMN reset_code_expiry DATETIME")
    print("Added column: reset_code_expiry")
else:
    print("Column already exists: reset_code_expiry")

conn.commit()
conn.close()
print("DONE: schema update complete.")
